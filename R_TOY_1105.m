...
variance = 0.0000005.*ones(nbFeatures,1);
...
for iter=1:n_iter
    ...
    sum=0;
    for i=1:M
        sum=sum+2*abs(sqrt((zz(i)-z0)^2+(yy(i)-y0)^2)-30)+abs(xx(i)-x0);
    end
    Return(iter)=exp(-sum/M);
    ...
    for i=1:min(iter,5)
        ...
        temp_explore_rm = (param_rm(:,j)-current_rm);
        temp_explore_lm = (param_lm(:,j)-current_lm);
        temp_explore_zb = (param_zb(:,j)-current_zb);
        ...
        % these simple sums
        param_nom_rm = param_nom_rm + temp_explore_rm*Return(j);
        param_nom_lm = param_nom_lm + temp_explore_lm*Return(j);
        param_nom_zb = param_nom_zb + temp_explore_zb*Return(j);
    ...
    c_rm = (60/(max(yy)-min(yy)))*circle_f*current_rm;
    c_lm = (60/(max(zz)-min(zz)))*circle_f*current_lm;
    c_zb = circle_f*current_zb;
